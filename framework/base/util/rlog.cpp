...
bool RLog::init(LogConfig &logConfig)
{
	...
	if (Base::RUtil::createDir(logDir))
	{
		QString fileName = logDir + QDir::separator() + Base::RUtil::getDate() + QString(Suffix);
...
void RLog::log(RLOG_LEVEL nLevel, const char *fileDesc, const char *functionDesc, int lineNum, const char* data, ...)
{
	...
	{
		QString recordInfo = QString("[%1]").arg(Base::RUtil::getTimeStamp());
