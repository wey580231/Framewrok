...
/*!
 * @brief 自定义绘制线条参数
 * @details 20201221为了实现在绘制线条时，增加更多的绘制控制权
 */
struct CustomGraphPaintParam{
    CustomGraphPaintParam():enableRangeLimit(false),limitValue(0),limitAreaColor(Qt::red){}

    bool enableRangeLimit;      /*!< 是否开启阈值限制绘制 */
    double limitValue;          /*!< 阈值值 */
    QColor limitAreaColor;      /*!< 阈值颜色 */
};
...
/*!
 * @brief 移除容器中第一个元素
 * @details 因QVector删除最前面一个元素时，需要将剩余的数据往前移动一个位置，这样频繁的使用会导致效率降低。
 *          此处只是将最前面的空余的数据数量+1，等到QVector长度过长时，会自动出发重新分配内存
 */
template <class DataType>
void QCPDataContainer<DataType>::removeFirst()
{
    if(mData.size() > 0 && mPreallocSize < mData.size() - 1){
        ++mPreallocSize;
...
protected:
  ...
  void drawPolyline(QCPPainter *painter, const QVector<QPointF> &lineData,QCP::CustomGraphPaintParam param = QCP::CustomGraphPaintParam()) const;
...
*/
template <class DataType>
void QCPAbstractPlottable1D<DataType>::drawPolyline(QCPPainter *painter, const QVector<QPointF> &lineData, QCP::CustomGraphPaintParam param) const
{
  ...
  } else
  {
    ...
    //NOTE wey 20201221 开启范围限制
    if(param.enableRangeLimit)
    {
        for(int i = segmentStart; i < lineDataSize - segmentStart - 1; i++){
            painter->save();
            QPointF pa = lineData.at(i);
            QPointF pb = lineData.at(i + 1);

            if((pa.y() < param.limitValue && pb.y() < param.limitValue)){
                painter->setPen(param.limitAreaColor);
                painter->drawLine(pa,pb);
            }else if((pa.y() > param.limitValue && pb.y() < param.limitValue) || (pa.y() < param.limitValue && pb.y() > param.limitValue)){
                double k = (pb.y() - pa.y()) / (pb.x() - pa.x());
                double x = pb.x() - (pb.y() - param.limitValue)/k;

                QPointF middlePoint(x,param.limitValue);

                if(pa.y() > param.limitValue){
                    painter->drawLine(pa,middlePoint);
                    painter->setPen(param.limitAreaColor);
                    painter->drawLine(middlePoint,pb);
                }else{
                    painter->drawLine(middlePoint,pb);
                    painter->setPen(param.limitAreaColor);
                    painter->drawLine(pa,middlePoint);
...
public:
  ...
  */
  ...
  //NOTE wey 20201221 增加对Y轴阈值设定，超过阈值的使用颜色高亮显示
  void enableLimitHighlight(bool enable);
  void setLimitValue(double value,QColor hightlightColor = Qt::red);
...
protected:
  ...
  bool mIsLimitHight;           /*!< 是否开启范围高亮显示 */
  double mLimitValue;           /*!< Y轴限定范围 */
  QColor mHightLightColor;      /*!< 超出限定值后的线条颜色 */
