...
namespace Base {
	...
	ActionContainer::ActionContainer(ActionContainerType type, QObject *parent) :
		QObject(parent)
	{
	...
	ActionContainerProxy::ActionContainerProxy(Id id, ActionContainerType type, QObject *parent) :m_id(id), ActionContainer(type, parent)
	{
	}
	...
	void ActionContainerProxy::addAction(Command *command, Id groupId)
	{
		...
		//NOTE 20200822 add时应添加至末尾
	//    QAction * tbeforeAction = insertLocation(groupIt);
		m_groups[groupIt - m_groups.constBegin()].items.append(command);

		insertAction(nullptr, command->action());
	}
	...
	void ActionContainerProxy::addMenu(ActionContainer *menu, Id groupId)
	{
		...
		insertMenu(nullptr, container->menu());
	}
	...
	MenuActionContainer::MenuActionContainer(Id id, QObject *parent) :m_menu(nullptr), ActionContainerProxy(id, C_Menu, parent)
	{
	...
	MenuBarActionContainer::MenuBarActionContainer(Id id) :m_menuBar(nullptr), ActionContainerProxy(id, C_MenuBar)
	{
